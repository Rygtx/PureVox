# PureVox - bundled Python 3.8 bootstrap script (Windows)
# Downloads the full prebuilt Python 3.8 from NuGet (includes dev headers/libs to build
# aimic.pyd and PyInstaller bundling) into packages\python38w\, independent of any
# system Python (which may be 3.14).
...
$pyVer = "3.8.10"   # last 3.8 python NuGet package

# 1. Download and unpack full Python 3.8 (NuGet, no admin needed; the real root is
...
Write-Host "PureVox bundled Python 3.8 ready: $py"
Write-Host "Use build_win.ps1 to package (it will pick up packages\python38w\python.exe)"