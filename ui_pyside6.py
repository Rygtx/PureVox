...
from pvplatform.system import (
    ...
    virtual_mic_ready, ensure_virtual_mic, remove_virtual_mic,
)
...
def _warn_vbcable_missing(download_url: str):
    """未检测到 VB-CABLE 驱动时引导下载弹框（风格同 48k 弹框）。"""
    import time as _time
    dlg = QDialog(None)
    dlg.setWindowTitle("PureVox")
    ...
    title = QLabel("需要安装 VB-CABLE 虚拟声卡：")
    title.setStyleSheet("font-size: 11pt;")
    layout.addWidget(title)

    tip = QLabel(
        "PureVox 不直接内置 VB-CABLE 驱动（作者版权限制），而是随包附带官方原版驱动包。"
        "点击\"下载驱动包\"获取官方 zip，放入 PureVox 所在目录（或 tools/ 文件夹），"
        "然后重启软件即可自动安装。"
    )
    ...
    btn_dl.clicked.connect(lambda _=False: QDesktopServices.openUrl(QUrl(download_url)))
    ...
    hint = QLabel("下载完成后重启 PureVox，即可自动安装。")
    hint.setStyleSheet("font-size: 10pt;")
    layout.addWidget(hint)
...
class MainApp:
    ...
    def _check_and_install_vbcable(self):
        """窗口显示后异步检测并安装 VB-Cable（仅 Windows；其它平台无虚拟声卡）。"""
        if not IS_WINDOWS:
            return
        from vbcable_installer import (
            _check_vbcable_installed, vbcable_setup_found, VBCABLE_DOWNLOAD_URL,
        )
        if _check_vbcable_installed():
            return

        if not vbcable_setup_found():
            _warn_vbcable_missing(VBCABLE_DOWNLOAD_URL)
            return

        from vbcable_installer import install_vbcable
        import threading

        def _do_install():
            result = install_vbcable(log_func=_state.logger.sys)
            if result:
                QTimer.singleShot(1000, lambda: _state.main_panel.refresh_devices() if _state.main_panel else None)
    ...
    def run(self):
        ...
        QTimer.singleShot(500, self._check_and_install_vbcable)
        if IS_LINUX:
            # 启动即创建虚拟麦克风（purevox_out null-sink + monitor），
            # 供其它应用当麦克风使用；等价 Windows VB-CABLE 但无需驱动。
            if ensure_virtual_mic(logger):
                # 创建后再刷新设备列表，让 "PureVox 虚拟麦克风" 出现在输出下拉中
                QTimer.singleShot(300, lambda: _state.main_panel.refresh_devices() if _state.main_panel else None)
        logger.sys("就绪")
        sys.exit(app.exec())
